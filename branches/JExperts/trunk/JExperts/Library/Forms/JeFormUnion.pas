...
The Original Code is: JeFormUnion.PAS, released on 2001-02-28.
...
uses
  ...
  StdCtrls, JvCombobox, ComCtrls, JvButton, ExtCtrls, Menus,
  JvListView, Buttons, JvSpeedButton, FileCtrl, JeExpert_Strings,
  ActnList, JvDialogs, JvExComCtrls, JvExControls;

type
  TfoUnion = class(TForm)
    Image1: TImage;
...
procedure TfoUnion.FormCreate(Sender: TObject);
begin
  if DirectoryExists(DelphiApis.PackPath) then
  begin
    ForceDirectories(DelphiApis.PackPath + 'User\Union');
    OpenDialog1.InitialDir := DelphiApis.PackPath + 'User\Union';
    SaveDialog1.InitialDir := DelphiApis.PackPath + 'User\Union';
  end;
end;
{*********************************************************************}

procedure TfoUnion.JvSpeedButton3Click(Sender: TObject);
var
  t: TListItem;
begin
...
procedure TfoUnion.JvSpeedButton4Click(Sender: TObject);
begin
  JvListView1.DeleteSelected;
end;
{*********************************************************************}

procedure TfoUnion.JvSpeedButton5Click(Sender: TObject);
type
  TRecordType = record
    name: string[255];
    ctype: integer;
...
//      end
//      else
//        Form.ToStatus(RCE_InvalidUnion);
//    end;
  end;
end;
{*********************************************************************}

procedure TfoUnion.JvSpeedButton6Click(Sender: TObject);
var
  i: integer;
begin
...
procedure TfoUnion.WideString1Click(Sender: TObject);
begin
  SetType((Sender as TMenuItem).Caption);
end;
{*********************************************************************}

procedure TfoUnion.Custom1Click(Sender: TObject);
var
  st: string;
begin
  st := '';
...
procedure TfoUnion.SetType(Value: string);
var
  i: integer;
begin
  for i := 0 to JvListView1.Items.Count - 1 do
...
procedure TfoUnion.PopupMenu1Popup(Sender: TObject);
var
  i: integer;
begin
...
procedure TfoUnion.JvSpeedButton1Click(Sender: TObject);
var
  i: integer;
begin
  //Test for bad inputs
  ...
  begin
    Beep;
    Form.ToStatus(RCE_NoUnionItems);
  ...
  begin
    ...
    if JvListView1.Items.Count < MAXBYTE then
      Add('case byte of')
    else
      Add('case integer of');
    for i := 0 to JvListView1.Items.Count - 1 do
      with JvListView1.Items[i] do
        Add('  ' + IntToStr(i) + ': (' + Caption + ': ' + SubItems[0] + ');');
    Add('end;');
    Add('');
