...
type
  ...
  private
    ...
    procedure SetWrapIndent(const Value: Boolean);

    property Settings: TCodeFormatterSettings read FSettings write FSettings;
    property FWrapIndent: Boolean read FWrapIndentInternal write SetWrapIndent;
  public
...
begin
  ...
  if not (rType in NoReservedTypes) then begin
//    Assert(False, 'trace');
    _CurrentToken.ExpressionCase := Settings.ReservedCase;
  end else if rType in StandardDirectives then begin
//    Assert(False, 'trace');
...
procedure TCodeFormatterFormatter.SetWrapIndent(const Value: Boolean);
begin
  if Value then
    Assert(False, 'WrapIndent set to True')
  else
    Assert(False, 'WrapIndent set to False');
  FWrapIndentInternal := Value;
end;
