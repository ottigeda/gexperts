...
type
  ...
  private
    ...
    procedure EngineSettingsToForm(const _Settings: TCodeFormatterEngineSettings);
...
procedure TfmCodeFormatterConfig.EngineSettingsToForm(const _Settings: TCodeFormatterEngineSettings);
resourcestring
...
begin
  ud_SpacePerIndent.Position := _Settings.SpacePerIndent;
  chk_IndentBegin.Checked := _Settings.IndentBegin;
  chk_IndentComments.Checked := _Settings.IndentComments;
  chk_IndentCompDirectives.Checked := _Settings.IndentCompDirectives;
  chk_IndentTry.Checked := _Settings.IndentTry;
  chk_IndentTryElse.Checked := _Settings.IndentTryElse;
  chk_IndentCaseElse.Checked := _Settings.IndentCaseElse;
  chk_UpperCompDirectives.Checked := _Settings.UpperCompDirectives;
  chk_UpperNumbers.Checked := _Settings.UpperNumbers;
  cmb_ReservedCase.ItemIndex := Ord(_Settings.ReservedCase);
  cmb_StandDirectives.ItemIndex := Ord(_Settings.StandDirectivesCase);
  cmb_IdentifiersCase.ItemIndex := Ord(_Settings.IdentifiersCase);
  chk_BlankProc.Checked := _Settings.BlankProc;
  chk_BlankSubProc.Checked := _Settings.BlankSubProc;
  chk_RemoveDoubleBlank.Checked := _Settings.RemoveDoubleBlank;
  with grid_Spacing do begin
    RowCount := 10;
    Cells[0, 0] := str_Description;
    Cells[1, 0] := str_Operators;
    Cells[2, 0] := str_Spacing;
    AddSpaceRow(1, str_Equals, ':=', _Settings.SpaceEqualOper);
    AddSpaceRow(2, str_MathOperators, str_MathOperatorsExample,
      _Settings.SpaceOperators);
    AddSpaceRow(3, str_Colon, ':', _Settings.SpaceColon);
    AddSpaceRow(4, str_SemiColon, ';', _Settings.SpaceSemiColon);
    AddSpaceRow(5, str_Comma, ',', _Settings.SpaceComma);
    AddSpaceRow(6, str_LeftParenthesis, '(', _Settings.SpaceLeftBr);
    AddSpaceRow(7, str_RightParenthesis, ')', _Settings.SpaceRightBr);
    AddSpaceRow(8, str_LeftBracket, '[', _Settings.SpaceLeftHook);
  ...
  chk_FeedAfterVar.Checked := _Settings.FeedAfterVar;
  chk_FeedElseIf.Checked := _Settings.FeedElseIf;
  chk_NoIndentElseIf.Checked := _Settings.NoIndentElseIf;
  chk_NoIndentVarDecl.Checked := _Settings.NoIndentVarDecl;
  chk_NoIndentUsesComma.Checked := _Settings.NoIndentUsesComma;
  chk_FeedBeforeElse.Checked := _Settings.FeedBeforeElse;
  chk_FeedBeforeEnd.Checked := _Settings.FeedBeforeEnd;
  chk_WrapLines.Checked := _Settings.WrapLines;
  ...
  ed_EndCommentOut.Text := string(_Settings.EndCommentOut);
  cmb_FeedRoundBegin.ItemIndex := Integer(_Settings.FeedRoundBegin);
  cmb_FeedRoundTry.ItemIndex := Integer(_Settings.FeedRoundTry);
